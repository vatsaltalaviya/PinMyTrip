...
const TravelPackageDetail = () => {
  ...
  const travelPackage = travelPackages.find(
    (packageItem) => packageItem.packageName === details)

  return (
    <div>
      <div className="max-w-6xl mt-15 mx-auto px-4 py-8">
        <h1 className="text-3xl font-bold mb-4">{travelPackage.packageName}</h1>

        <img
          src={travelPackage.image}
          alt={travelPackage.packageName}
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
          <div>
            ...
            <ul className="list-disc list-inside text-gray-600">
              <li>{travelPackage.keyHighlights[0]}</li>
              <li>{travelPackage.keyHighlights[1]}</li>
              <li>{travelPackage.keyHighlights[2]}</li>
            </ul>
          </div>
          ...
          <div>
            <h1 className="text-3xl font-semibold mt-5 ">
              Starting From ₹
              <span className="font-bold">{travelPackage.amount}</span>
            </h1>
          </div>
          <div className="bg-white px-8 md:px-10 py-1 md:py-0 md:pl-2 border h-13 rounded-2xl w-72 md:w-60">
            <button
              onClick={() => {
                navigate(`/packageBook/${travelPackage.packageName}`);
                scrollTo(0, 0);
              }}
              className="text-2xl md:text-xl font-medium flex justify-between items-center"
            >
              Booking Now{" "}
              <span className="text-xl md:text-2xl font-medium p-1.5 ml-3 rounded-xl">
