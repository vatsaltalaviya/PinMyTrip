...
const Booking = () => {
  ...
  const{destinations} =  useContext(destinationDataContext);
    ...
    const destinationInfo = destinations.find((city) => city.cityName === Book);
    ...
    <div className="p-8 mt-15">
      ...
      <div className="flex flex-col md:flex-row  mt-10">
        <div className="w-full md:w-2/3 rounded-2xl h-56 md:h-[70vh] overflow-hidden">
          <img className="h-full w-full" src={destinationInfo.image} alt="" />
        </div>
        <div className="w-full md:w-1/3 flex flex-col p-5 items-start pl-0 md:pl-10">
          <h1 className="text-2xl font-medium">{destinationInfo.cityName}</h1>
          ...
          <div className="flex flex-col md:flex-row gap-3 mt-5">
            <span className="px-5 py-1.5 rounded-full border border-black">
              ...
              {destinationInfo.recommendedDays} days
            </span>
            <span className="px-5 py-1.5 rounded-full border border-black">
              <i className="ri-calendar-2-line mx-1"></i>
              {destinationInfo.bestMonthsToVisit}
            </span>
          </div>
          <div className="flex flex-col md:flex-row gap-3 mt-5">
            <span className="px-5 py-1.5 rounded-full border border-black">
            ...
            <form action="">
              ...
              <div className="mt-3">
                <h1 className="text-2xl font-semibold">
                  Starting From ₹<span>{destinationInfo.amount}</span>
                </h1>
                <button
